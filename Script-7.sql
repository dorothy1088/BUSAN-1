...
update article set id = 1 where id = 0 limit 1 ; -- limit은 갯수 제한 
-- article 테이블 조회(*)
select * from article; 
-- id가 0인것을 id 2로 바꾼다.
...
desc article; 
-- id 칼럼에 UNSIGNED 속성을 추가하세요. unsigned는 음수는 안됨 양수만 가능
alter table article modify column id bigint unsigned not null auto_increment;
-- article 테이블 구조확인(desc)
desc article; 
-- 작성자(writer) 칼럼을 title 칼럼 다음에 추가해주세요. 속성은 순서 없고 컬럼은 순서 있음
...
select * from article where nickname != '무명' and hit <=55;
